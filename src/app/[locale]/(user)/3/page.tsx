        ...
        events: {
          ...
          onStateChange: (event: { data: number }) => {
            if (event.data === window.YT.PlayerState.PLAYING) {
              // Use all states for robust control
              if (timer) {
                clearInterval(timer);
                setTimer(null);
              }
              const timerInterval = setInterval(() => {
                const current = Math.round(playerInstance.getCurrentTime());
                // Debug: log current time and states
                // console.log('Current:', current, 'currentstop:', currentstop, 'hidemenu:', hidemenu, 'open:', open, 'selected:', selected);
                if (videoStops.includes(current) && !currentstop && !hidemenu && !open && selected === 0) {
